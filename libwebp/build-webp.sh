#!/bin/bash
# Build cwebp + dwebp as universal static executables for macOS.
# Called from Xcode's "Run Script" build phase.
#
# Builds libpng and libjpeg-turbo from the libjxl third_party sources
# to ensure static libraries are available for both architectures.
...
if [ -f "$MARKER" ] && [ -f "$OUTPUT_DIR/cwebp" ] && [ -f "$OUTPUT_DIR/dwebp" ]; then
    ...
    if [ "$MARKER_TIME" -ge "$CMAKE_TIME" ]; then
        echo "webp: already built (up to date)"
        exit 0
    fi
fi
...
build_deps() {
    ...
    echo "webp: building dependencies (zlib, libpng, libjpeg-turbo) for $ARCH..."
...
build_arch() {
    ...
    cmake --build "$BUILD_DIR" --target cwebp dwebp --config Release -- -j"$(sysctl -n hw.ncpu)" 2>&1 | tail -5
}
...
echo "webp: creating output binaries..."

for TOOL in cwebp dwebp; do
    INPUTS=()
    for ARCH in "${ARCHS[@]}"; do
        TOOL_PATH="$BUILD_ROOT/$ARCH/examples/$TOOL"
        if [ ! -f "$TOOL_PATH" ]; then
            TOOL_PATH="$BUILD_ROOT/$ARCH/$TOOL"
        fi
        if [ ! -f "$TOOL_PATH" ]; then
            echo "ERROR: Could not find $TOOL for $ARCH"
